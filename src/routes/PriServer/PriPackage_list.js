...
import CreateEditForm from './PriPackage_edit';
import CreateFindForm from './PriPackage_find';
...
@Form.create()
export default class PriPackage extends PureComponent {
  state = {
    modalVisible: false,
    modalTitle: '',
    formValues: {},
    ...
    Modal.confirm({
      ...
      onOk() {
        dispatch({
          ...
          path: 'priPackage/remove',
          payload: { priPackageId: record.priPackageId },
          callback: cb
        });
      },
      onCancel() {}
  ...
  getDataForUpdate = record => {
    ...
    dispatch({
      ...
      path: 'priPackage/getDataForUpdate',
      payload: { priPackageId: record.priPackageId }
    });
    this.setState({
      modalTitle: '修改',
  ...
  detail = (record, uri) => {
    ...
    dispatch({
      ...
      path: uri,
      payload: {
        priPackageId: record.priPackageId
      }
    });
  };
      ...
      },
      {
        ...
        render: (text, record) => (
          <Fragment>
            ...
            <a onClick={() => this.detail(record, '/vipServer/priPackageItem')}>权益包明细</a>
          </Fragment>
        )
      }
    ];
