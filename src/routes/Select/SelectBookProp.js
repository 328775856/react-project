...
export default class SelectBookProp extends React.Component {
  ...
  query = (params, page) => {
    const { dispatch, bookPropId } = this.props;
    this.setState({ bookPropId: bookPropId });
    dispatch({
  ...
  render() {
    ...
    const rowSelection = {
      ...
      onChange: (selectedRowKeys, selectedRows) => {
        ...
        this.setState({ bookPropId: selectedRows[0].bookPropId });
      },
      ...
      getCheckboxProps: record => {
        const { bookPropId } = this.state;
        return {
          checked: record.bookPropId === bookPropId
        };
      }
    };
