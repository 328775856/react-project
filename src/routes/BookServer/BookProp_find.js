...
return (
  <Form onSubmit={formQuery} layout="inline">
    <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
      ...
      <Col md={8} sm={24}>
        <FormItem label="是否可借">
          {form.getFieldDecorator('isLend', {initialValue: '-1'})(<Select style={{ width: '150px' }}>{opt1}</Select>)}
        </FormItem>
      </Col>
      <Col md={8} sm={24}>
        <FormItem label="借阅天数">
      ...
      <Col md={8} sm={24}>
        <FormItem label="是否限制">
          {form.getFieldDecorator('isLimit', {initialValue: '-1'})(<Select style={{ width: '150px' }}>{opt1}</Select>)}
        </FormItem>
      </Col>
      <Col md={8} sm={24}>
        <span>
