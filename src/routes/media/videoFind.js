...
function CreateFindFrom(props, formQuery, formReset, videoGroup) {
  const { form } = props;
  ...
  return (
    <Form onSubmit={formQuery} layout="inline">
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={8} sm={24}>
          <FormItem label="分组">
            {form.getFieldDecorator('mediaVideoGroupId')(<Select placeholder="" style={{width : '150px'}}>{videoGroup}</Select>)}
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="名称">
