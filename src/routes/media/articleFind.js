...
<Form onSubmit={formQuery} layout="inline">
    <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                    <Col md={8} sm={24}>
            <FormItem label="分组">
                {form.getFieldDecorator('mediaArticleGroupId')(<Select placeholder="" style={{width : '150px'}}>{articleGroup}</Select>)}
            </FormItem>
            </Col>
                    <Col md={8} sm={10}>
            <FormItem label="标题">
                {form.getFieldDecorator('articleTitle')(<Input placeholder="" />)}
            </FormItem>
            </Col>
                <Col md={8} sm={24}>
        <span>
