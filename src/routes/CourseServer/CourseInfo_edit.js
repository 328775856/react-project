...
export default class CreateEditForm extends PureComponent {
  ...
  callSelectAuthorReturn = record => {
    if (record != null) {
      const { dispatch } = this.props;
      const myFormData = {
        courseAuthorId: record[0].courseAuthorId,
  ...
  render() {
    ...
    const okHandle = () => {
      form.validateFields((err, fieldsValue) => {
        if (err) return;
        form.resetFields();
        if (formData.courseInfoId >= 0) {
          fieldsValue.courseInfoId = formData.courseInfoId;
          update(fieldsValue);
        } else {
          add(fieldsValue);
        }
      ...
      <Modal title={title} visible={modalVisible} onOk={okHandle} onCancel={() => closeModal()}>
        <FormItem {...formItemLayout} label="课程类型">
          {form.getFieldDecorator('courseType', {
            initialValue: formData.courseType || '',
            rules: [
              {
                required: true,
                message: '请输入课程类型(字典1:单音频 2:多音频)...',
              },
            ],
          })(<Input />)}
        </FormItem>
        ...
        <FormItem {...formItemLayout} label="作者id" style={{display:'none'}}>
          <Row gutter={0}>
            <Col span={22}>
              {form.getFieldDecorator('courseAuthorId', {
                initialValue: formData.courseAuthorId || '',
                rules: [
                  {
                    required: true,
                    message: '请输入作者id...',
        ...
        <FormItem {...formItemLayout} label="是否首页推荐">
          {form.getFieldDecorator('isRecommend', {
            initialValue: formData.isRecommend || '',
            rules: [
              {
                required: true,
        ...
        <SelectAuthor {...parentMethodsForAuthor} modalVisible={this.state.authorModalVisible} />
        <SelectTag {...parentMethodsForTag} modalVisible={this.state.tagModalVisible} />
        <SelectColumn {...parentMethodsForColumn} modalVisible={this.state.columnModalVisible} />
      </Modal>
    );
  }
}
