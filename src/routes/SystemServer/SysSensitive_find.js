...
function CreateFindForm(props,formQuery,formReset,isEmptyObject,curState) {
  ...
  let options = '';
   if(!isEmptyObject(curState.options)){
     options = curState.options.map(item => <Option key={item.sensitiveTypeId}>{item.typeName}</Option>);
   }
    ...
    <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
      <Col md={8} sm={24}>
        ...
        <FormItem label="敏感词分类">
          {form.getFieldDecorator('sysSensitiveTypeId')(<Select placeholder="" style={{width : '150px'}}>{options}</Select>)
          }
        </FormItem>
