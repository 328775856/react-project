  ...
  return (
    <Form onSubmit={formQuery} layout="inline">
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        ...
        <Col md={8} sm={24}>
          <FormItem label="是否检查">
            {form.getFieldDecorator('paramCheck', {initialValue: '-1',})(<Select style={{ width: '150px' }}>{opt1}</Select>)}
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="有效开始日期">
            {form.getFieldDecorator('beginDate')(<DatePicker format="YYYYMMDD" />)}
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="有效结束日期">
            {form.getFieldDecorator('endDate')(<DatePicker format="YYYYMMDD" />)}
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <span>
