...
function CreateFindFrom(props,formQuery,formReset,isEmptyObject,curState) {
  ...
  if(!isEmptyObject(curState.options)){
    options = curState.options.map(item => <Option key={item.productTypeId}>{item.productTypeName}</Option>);
  }
const { form } = props;
    ...
    <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
            ...
            <FormItem label="产品名称">
                {form.getFieldDecorator('productName')(<Input placeholder="" />)}
                    ...
                    <Col md={8} sm={24}>
            <FormItem label="产品类型">
              {form.getFieldDecorator('orderProductTypeId')(<Select placeholder="" style={{width : '150px'}}>{options}</Select>)
              }
            </FormItem>
            </Col>
