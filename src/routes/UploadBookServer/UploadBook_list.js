  ...
  ,{
    ...
    render(text, record) {
      const dict = record => {
        ...
        if(options.dictDatas) {
          const dictOpt = JSON.parse(options.dictDatas);
          const myArray = dictOpt["33"];
          for(const k in myArray) {
            if (record.uploadType === myArray[k].itemNo) {
              res = myArray[k].itemLabel;
              break;
            }
          }
...
return (
  <PageHeaderLayout>
    ...
    <CreateAuditForm {...parentMethods} modalVisible={modalAuditVisible} formData={tableData.formData} title={modalAuditTitle} options={options} />
  </PageHeaderLayout>
);
}
}
