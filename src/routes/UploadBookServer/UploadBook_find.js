  ...
  return (
    <Form onSubmit={formQuery} layout="inline">
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        ...
        <Col md={8} sm={24}>
          <FormItem label="图书ID">{form.getFieldDecorator('bookId')(<InputNumber />)}</FormItem>
        </Col>
        ...
        <Col md={8} sm={24}>
          <FormItem label="图书格式">
            {form.getFieldDecorator('bookStyleId', { initialValue: '-1' })(
              <Select>{opt11}</Select>
            )}
          </FormItem>
        </Col>
        ...
        <Col md={8} sm={24}>
          <FormItem label="上传时间">
            {form.getFieldDecorator('createTimeStart')(
              <DatePicker format="YYYY-MM-DD HH:mm:ss" showTime={{}} placeholder="开始时间" />
            )}
            &nbsp;-&nbsp;
            {form.getFieldDecorator('createTimeEnd')(
              <DatePicker format="YYYY-MM-DD HH:mm:ss" showTime={{}} placeholder="结束时间" />
            )}
          </FormItem>
        </Col>
        {/* <Col md={8} sm={24}>
