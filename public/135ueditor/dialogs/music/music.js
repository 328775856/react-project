...
(function() {
  ...
  Music.prototype = {
    init: function() {
      ...
      var me = this;
      var audio = jQuery(dialog.anchorEl)
        .find('audio')
        .get(0);
      if (audio) {
        if (
          !audio.tagName ||
          (audio.tagName.toLowerCase() != 'audio' && !audio.getAttribute('src')) ||
          !audio.src
        )
          return;
    ...
    exec: function() {
      ...
      if ($G('ImageUrl').value) {
        ...
        if (parent.current_active_135item != null && audio) {
          if (
            (!audio.tagName || audio.tagName.toLowerCase() != 'audio') &&
            (!audio.getAttribute('src') || !audio.src)
          )
            return;
          ...
          var html =
            "<section class='135editor' style='text-align:center;'><audio " +
...
(function() {
  ...
  QQMusic.prototype = {
    ...
    dosearch: function() {
      ...
      if ('' == $.trim(e)) return !1;
      (e = encodeURIComponent(e)), d._sent(e);
    },
    doselect: function(a) {
      var b = this;
    ...
    onpageclick: function(c) {
      for (var d = 1; d <= a.length; d++)
        $('#page' + d).attr('class', 'pageoff'), $('#panel' + d).attr('class', 'paneloff');
      $('#page' + c).attr('class', 'pageon'), $('#panel' + c).attr('class', 'panelon');
    },
    ...
    _sent: function(a) {
      ...
      $('#resultBar').html(function() {
        ...
        $.ajax({
          url:
            b.dataUrl +
            'jsonCallback=success_jsonpCallback&remoteplace=txt.weixin.officialaccount&w=' +
            a +
            '&platform=weixin&perpage=' +
            b.total +
            '&curpage=1',
          dataType: 'jsonp',
          jsonp: 'callback',
    ...
    _getUrl: function(a) {
      var b = this,
        d =
          ...
          encodeURIComponent(c.albumname) +
          '&artist=' +
          encodeURIComponent(c.singername) +
          '&extra=' +
          encodeURIComponent(c.albumname) +
          '&autoPlay=' +
          a +
    ...
    _getTryingElem: function() {
      for (var a = $('#listPanel span'), b = 0; b < a.length; b++)
        if ('m-trying' == a[b].className) return a[b];
      return null;
    },
    ...
    _rebuildData: function(a) {
      for (var b, c = this, d = [], e = c.pageSize, f = 0; f < a.length; f++)
        (f + e) % e == 0 && ((b = []), d.push(b)), b.push(a[f]);
      return d;
    },
    _renderTemplate: function(c) {
      ...
      var e = [],
        f = [],
        g = [];
      e.push('<div id="listPanel" class="listPanel">'), f.push('<div class="page">');
      for (var h, i = 0; (h = c[i++]); ) {
        b.push('panel' + i),
          a.push('page' + i),
          1 == i
            ? (e.push('<div id="panel' + i + '" class="panelon">'),
              1 != c.length &&
                g.push(
                  '<div id="page' +
                    i +
                    '" onclick="qqmusic.onpageclick(' +
                    i +
                    ')" class="pageon">' +
                    i +
                    '</div>'
                ))
            : (e.push('<div id="panel' + i + '" class="paneloff">'),
              g.push(
                '<div id="page' +
                  i +
                  '" onclick="qqmusic.onpageclick(' +
          ...
          e.push('<label for="radio-' + i + '-' + k + '" class="m-m">'),
            e.push(
              '<input type="radio" id="radio-' +
                ...
                '-' +
                k +
                '" name="musicId" class="m-l" onclick="qqmusic.doselect(' +
                (d.pageSize * (i - 1) + (k - 1)) +
                ')"/>'
            ),
            e.push('<i></i>'),
            e.push(
              '<span class="m-t" title="' +
                j.songname +
                '">' +
                d._getMaxText(j.songname) +
                '</span>'
            ),
            e.push(
              '<span class="m-s" title="' +
                j.singername +
                '">' +
                d._getMaxText(j.singername) +
                '</span>'
            ),
            e.push(
              '<span class="m-z" title="' +
                j.albumname +
                '">' +
                d._getMaxText(j.albumname) +
                '</span>'
            ),
            e.push(
              '<span class="m-try" onclick="qqmusic.doselect(' +
                (d.pageSize * (i - 1) + (k - 1)) +
                ');qqmusic.listenTest(this)"></span>'
            ),
            e.push('</label>');
        }
      ...
      return (
        g.reverse(), f.push(g.join('')), e.push('</div>'), f.push('</div>'), e.join('') + f.join('')
      );
    },
    exec: function() {
      var a = this;
      if (null != c) {
        ...
        var b =
            ...
            encodeURIComponent(c.f.split('|')[3]) +
            '&music_name=' +
            encodeURIComponent(c.songname),
          d = c.f.split('|')[22];
        d =
          '/' +
          d.slice(d.length - 2, d.length - 1) +
          '/' +
          d.slice(d.length - 1, d.length) +
          '/' +
          d +
          '.jpg';
        var e = {
