...
(function($) {
  ...
  $[pluginName] = function(element, options) {
    ...
    var correctValuePerc = function(value) {
      ...
      value =
        Math.floor(value / accuracy) * accuracy +
        Math.round((value % accuracy) / accuracy) * accuracy;
      if (value > 100) {
        return 100;
      }
      return value;
    ...
    var startMoveMarker = function(e) {
      ...
      $(document).on('mousemove.sliderMarker  touchmove.sliderMarker', function(event) {
        movingMarker(event);
      });
      $(document).on('mouseup.sliderMarker touchend.sliderMarker', function() {
        $(document).off('mousemove.sliderMarker');
        $(document).off('mouseup.sliderMarker');
        $(document).off('touchmove.sliderMarker');
        $(document).off('touchend.sliderMarker');
    ...
    var movingMarker = function(event) {
      ...
      if (
        ...
        if (vertical) {
          cursorPos = _touch.pageY - sliderOffset;
        } else {
          cursorPos = _touch.pageX - sliderOffset;
        }
      } else {
